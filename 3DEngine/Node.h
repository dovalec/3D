...
public:
    ...
    inline void GetTranslation(float & tx, float & ty, float & tz)
	{
		tx = mTranslation.x;
        ty = mTranslation.y;
        tz = mTranslation.z;
    ...
    inline void GetRotation(float & rx, float & ry, float & rz, float & rw)
	{
		rx = mRotation.x;
        ry = mRotation.y;
        rz = mRotation.z;
	...
	inline void SetScale(PVRTVec3 scale)
	{
		mScale = scale;		
		UpdateMatrices();
	}
	...
	inline void UpdateInverseWorldTransform()
	{
		mUpdateInverse = true;
	}


	void SetExternalUpdateModeTraverse(bool mode);
	...
	void SetVisibleChildren(bool flag);
	...
	virtual void UpdateAfterLoadingModel();
	virtual void Update();
	virtual void Render();
	virtual void Apply();
    ...
    inline void GetWorldTranslation(float & tx, float & ty, float & tz)
	{
		tx = mWorld.f[12];
        ty = mWorld.f[13];
        tz = mWorld.f[14];
