...
void Node::SetVisibleChildren(bool flag)
{
	mVisible = flag;
	...
	{
		pChild->SetVisibleChildren(flag);
	}
...
void Node::SetExternalUpdateModeTraverse(bool mode)
{
	ListIterator iter(mChildren);
	while (Node * pChild = (Node*)iter.GetPtr())
	{
		pChild->SetExternalUpdateModeTraverse(mode);
	}

	SetExternalUpdateMode(mode);
}

void Node::UpdateWithoutChildren()
{
	if (mUpdateLocal)
	{
		UpdateLocalMatrix();
...
void Node::UpdateAfterLoadingModel()
{
	UpdateLocalMatrix();	
	UpdateWorldTransform();
	...
	while (Node * pChild = (Node*)iter.GetPtr())
	{
		pChild->UpdateAfterLoadingModel();
	}
...
PVRTVec2 Node::GetProjected()
{
	...
	Camera * pCamera = pCameraManager->GetCurrentCamera();
...
{
	if (mParent == NULL || mExternalUpdateMode )
	{
		mWorld = mLocal;
	}
	else
